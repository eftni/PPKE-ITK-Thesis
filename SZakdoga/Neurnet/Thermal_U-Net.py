#######################################
# http://openaccess.thecvf.com/content_cvpr_2018_workshops/papers/w21/Berg_Generating_Visible_Spectrum_CVPR_2018_paper.pdf
#
#
#
#
...
def gray_block(prev, filters, size, stride):
    conv1 = Conv2D(filters, size, activation='relu', strides=stride, padding="same")(prev)
    out = BatchNormalization()(conv1)
    return out

def green_block(prev, filters, size):
...
def blue_block(prev, filters, size):
    convT = Conv2DTranspose(filters, size, activation='relu', strides=2, padding="same")(prev)
    conv1 = Conv2D(filters, size, activation='relu', strides=1, padding="same")(convT)
