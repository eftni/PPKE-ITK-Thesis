...
class DSSIMObjective:
    ...
    def __call__(self, y_true, y_pred):
        ...
        patches_pred = tf.image.extract_patches(images=y_pred, sizes=kernel, strides=kernel, rates=[1,1,1,1],
                                          padding='VALID')
        patches_true = tf.image.extract_patches(images=y_true, sizes=kernel, strides=kernel, rates=[1,1,1,1],
                                          padding='VALID')

        # Reshape to get the var in the cells
        #print(self.__int_shape(patches_pred))
        #bs, w, h, ch1, ch2, ch3 = self.__int_shape(patches_pred)
        #patches_pred = K.reshape(patches_pred, [-1, w, h, ch1 * ch2 * ch3])
        #patches_true = K.reshape(patches_true, [-1, w, h, ch1 * ch2 * ch3])
        # Get mean
